...
import (
	...
	"github.com/akhidrb/oapi-codegen/pkg/codegen"
	"github.com/akhidrb/oapi-codegen/pkg/util"
)
...
func main() {
	...
	flag.BoolVar(
		&flagOutputConfig, "output-config", false,
	...
	flag.StringVar(
		&flagGenerate, "generate", "types,client,server,spec",
	...
	flag.StringVar(
		&flagImportMapping, "import-mapping", "", "A dict from the external reference to golang package path",
	)
	flag.StringVar(
		&flagExcludeSchemas, "exclude-schemas", "",
...
func newConfigFromOldConfig(c oldConfiguration) configuration {
	...
	templates, err := loadTemplateOverrides("api/v1/templates/")
	if err != nil {
